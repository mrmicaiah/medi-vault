...
## Current Session Work (March 29-30, 2026)
...
### Frontend Changes
1. **ApplyPage** (`/apply/:agencySlug`) - Public application page with:
   - Agency branding fetched from API
   - Location selection
...
- Application: DELETED (to allow testing fresh applicant flow)

---

## CURRENT ISSUE - NEEDS FIXING

**Error:** `column documents.expires_at does not exist`

The admin dashboard calls an endpoint that queries `documents.expires_at` but this column doesn't exist.
...
**Also check:** The documents table schema - may need additional columns.

**CORS Status:** Currently set to allow all origins (`"*"`) for debugging. Should be restricted back to:
```
...
1. Fix documents table columns
2. Restrict CORS back to specific origins (remove "*")
